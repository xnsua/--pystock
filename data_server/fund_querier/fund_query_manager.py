...
from data_server.fund_querier import easy_money_querier
from data_server.fund_querier import hexun_querier


class FundQuerier:
    @classmethod
    def query_or_update_all(cls):
        is_fund_codes_outdated = is_file_outdated(
            cls._get_fund_code_list_path(), span=timedelta(days=30))
        if is_fund_codes_outdated:
            fcodes = easy_money_querier.wget_all_stockfund_code()
        else:
            df = pandas.read_csv(cls._get_fund_code_list_path(), dtype=str,
                                 index_col=0)
        ...
        for fcode in fcodes:
            ...
            try:
                if is_file_outdated(cls.get_fund_info_pathname(fcode),
                                    timedelta(days=30)):
                    cls.query_and_save_fund_info(fcode)
                else:
                    pass  # logger.info('%s fund info is up to date')
            except Exception as e:
                logger.warn('Query Fund info %s failed, Msg : %s', fcode,
                            str(e))
            try:
                if is_file_outdated(cls.get_fund_value_pathname(fcode),
                                    timedelta(days=30)):
                ...
                logger.warn('Query Fund value %s failed, Msg: %s', fcode,
                            str(e))
    ...
    def _get_fund_info_save_path(fund_code: str):
        path = Path(myconfig.fund_data_path)
        save_path = path / fund_code
        return str(save_path)

    @staticmethod
    def _get_fund_code_list_path():
        return str(Path(myconfig.fund_data_path) / 'fund_codes.csv')
