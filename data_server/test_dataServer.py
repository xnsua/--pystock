...
class TestDataServer(TestCase):
    ...
    def test_interval_pre_trade1(self):
        ...
        kwargs = {
            ktc_.push_realtime_interval: hp.dt.timedelta(seconds=1),
            ktc_.trade1_timedelta: (hp.dt.timedelta(seconds=60), hp.dt.timedelta(seconds=60)),
            ktc_.trade2_timedelta: (hp.dt.timedelta(seconds=30), hp.dt.timedelta(seconds=30))}
        data_server_thread = threading.Thread(
            target=thread_data_server_loop,
            args=(self.trade_context,),
            kwargs=kwargs)

        trade_manager_queue = self.trade_context.queue_dict[ktc_.id_trade_manager]
        ...
        self.trade_context.send_msg(ktc_.id_data_server, ktc_.msg_quit_loop, None)
        ...
        for val in data_server_queue.queue:
            assert val.operation == ktc_.msg_push_realtime_stocks
    ...
    def test_interval_between_trade(self):
        ...
        kwargs = {
            ktc_.push_realtime_interval: hp.dt.timedelta(seconds=1),
            ktc_.trade1_timedelta: (hp.dt.timedelta(seconds=60), hp.dt.timedelta(seconds=0)),
            ktc_.trade2_timedelta: (hp.dt.timedelta(seconds=30), hp.dt.timedelta(seconds=30))}
        data_server_thread = threading.Thread(
            target=thread_data_server_loop,
            args=(self.trade_context,),
    ...
    def test_interval_after_trade(self):
        ...
        kwargs = {
            ktc_.push_realtime_interval: hp.dt.timedelta(seconds=1),
            ktc_.trade1_timedelta: (hp.dt.timedelta(seconds=60), hp.dt.timedelta(seconds=0)),
            ktc_.trade2_timedelta: (hp.dt.timedelta(seconds=30), hp.dt.timedelta(seconds=0))}
        data_server_thread = threading.Thread(
            target=thread_data_server_loop,
            args=(self.trade_context,),
            kwargs=kwargs)
