...
def drop_after_count1(code, days=None):
    ...
    for index, day in enumerate(days):
        if stock != 0:
            money = stock * ddr_pv.open(code, day)
            stock = 0
        elif drop_count == 2:
            buy_count += 1
            stock = money / ddr_pv.open(code, day)
            money = 0
        else:
            pass

        if ddr_pv.open(code, day) > ddr_pv.close(code, day):
            drop_count += 1
        else:
            drop_count = 0
        print(money)
    ...
    if stock != 0:
        money = stock * ddr_pv.close(code, days[-1])
    yield_ = money / 10000
    year_yield = yield_ ** (245 / buy_count)
    original_yield = ddr_pv.close(code, ddr.days[-1]) / ddr_pv.open(code, ddr.days[0])
...
def drop_after_count2(code):
    ...
    for index, day in enumerate(ddr.days):
        ...
        if ddr_pv.close(code, ddr.days[index - 1]) > ddr_pv.close(code, day):
            drop_count += 1
        else:
            drop_count = 0

    if stock != 0:
        money = stock * ddr_pv.close(code, ddr.days[-1])
    yield_ = money / 100000
    year_yield = yield_ ** (245 / buy_count)
    original_yield = ddr_pv.close(code, ddr.days[-1]) / ddr_pv.open(code, ddr.days[0])
    year_yield_base = original_yield ** (245 / len(ddr.days))
...
def drop_after_count3(code):
    ...
    for index, day in enumerate(ddr.days):
        ...
        if ddr_pv.open(code, ddr.days[index - 1]) > ddr_pv.open(code, day) \
                and ddr_pv.close(code, ddr.days[index - 1]) > ddr_pv.close(code, day):
            drop_count += 1
        else:
            drop_count = 0
...
def compare():
    ...
    days = ddr_pv.ddr_of('sh510050').days[0:30]
    print(ddr_pv.ddr_of('sh510050').df)
    codes = ['sh510050']
    for code in codes:
